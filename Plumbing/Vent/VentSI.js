...
function addNode(xdoc) {                                    //note the xdoc.
    ...
    for (i = 0; i < n; i++) {
        ...
        var inTextmm = (inText * 25).toFixed(0);
        //alert(inTextmm);             
        var newText = document.createTextNode(inTextmm);      //create the text node each time.Pay attention to text node and element node
        ...
        if (y - DFU > -0.1) {                                //Warning!! CANNOT USE y>DFU
            ...
            for (j = 0; j < l; j++) {
                ...
                if (z1 - TDL > -0.1) {                                               //Warning!! CANNOT USE z1>TDL
                    ...
                    document.getElementById("vd").innerHTML = (z2*25).toFixed(0);
                    ...
                    document.getElementById("TDLm").innerHTML = (z4*0.3048).toFixed(0);                    
                    return false;
                }
            }
        }
...
function getSumpVentSize(q, ln) {                               //
    ...
    q = q * 15.85;                                              //convert L/s to gpm so that Imperist System data can be used
    ln = ln / 0.3048;                                           //convert meter to feet
    ...
    for (i = 0; i < n; i++) {
        ...
        for (j = 0; j < m; j++) {
            ...
            if (y-q>-0.1 & z-ln>-0.1) {                         //comparing flow and TDL
                ...
                var vdmm = vd * 25;
                document.getElementById("vsd").innerHTML = vdmm;
                document.getElementById("Qmsp").innerHTML = (y/15.85).toFixed(1);
                document.getElementById("TDLmsp").innerHTML =parseInt(z*0.3048);                 
                return false;
            }                     
        }
    }
