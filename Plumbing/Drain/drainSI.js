...
function initAll() {
    if (window.XMLHttpRequest) {
        ...
        if (window.ActiveXObject) {
            try {
                ...
                xst = new ActiveXObject("Microsoft.XMLHTTP");                
            }
            catch (e) { }
        }
    }
    ...
    if (xhr) {
        ...
        xhr.open("GET", "xml/drainsizeSI.xml", true);                       //open the xml file and goto populateMenu function
        xhr.send(null);
    }
    else {
...
function getDrainSize(u){
    ...
    for (var j = 1; j < n; j++) {
        ...
        if (hbd[j][1] > u-1) {
            var hd = hbd[j][0];
            document.getElementById("dhd").innerHTML = hd*50;
