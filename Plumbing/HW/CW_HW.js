...
function convertFUtoQ(u) { 
    ...
    for (i = 1;i<m;i++){
        ...
        if (fut[i][0] > u & fut[i-1][0] <= u) {
            ...
            document.getElementById("Qt").innerHTML = qt.toFixed(1);
            convertFUvtoQ(u)
            return false;
        }
    }
    ...
    for (var j = 1; j < n; j++) {
        ...
        if (fuv[j][0] > u & fuv[j - 1][0] <= u) {
            ...
            if (u < 5) {
                ...
                document.getElementById("Qv").innerHTML = qv.toFixed(1);
            } 
            return false;
        }
    }     
...
function power(q, tc, th,e){
    e = e / 100;
    var hp = 500.72 * q * (th - tc) / e / 2544;     
    var kw = 500.72 * q * (th - tc) / e / 3412;
    var mbh = 500.72 * q * (th - tc) /e/ 1000;  
    var lb = 500.72 *q * (th - tc) /e/ 960;
    document.getElementById("mbh").innerHTML = mbh.toFixed(1);
    document.getElementById("kw").innerHTML = kw.toFixed(1);    
    document.getElementById("hp").innerHTML = hp.toFixed(1);
    document.getElementById("lbh").innerHTML =lb.toFixed(1);
    return false;
}
